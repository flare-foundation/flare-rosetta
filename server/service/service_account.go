...
func (s AccountService) AccountBalance(
	...
	flrBalance, err := s.client.BalanceAt(ctx, address, header.Number)
	if err != nil {
		return nil, wrapError(errClientError, err)
	}
	...
	if len(req.Currencies) == 0 {
		balances = append(balances, mapper.FlareAmount(flrBalance))
	}
	...
	for _, currency := range req.Currencies {
		...
		if !ok {
			...
			return nil, wrapError(errCallInvalidParams, errors.New("non-flr currencies must specify contractAddress in metadata"))
		}
